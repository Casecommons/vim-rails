...
" Returns the subset of ['test', 'spec', 'jasmine', 'cucumber'] present on the app.
function! s:app_test_suites() dict
  return filter(['test','spec','jasmine','cucumber'],'self.has(v:val)')
endfunction
...
function! s:BufFinderCommands()
  ...
  if rails#app().has('jasmine')
    call s:addfilecmds("jasmine")
  endif
  if rails#app().has('cucumber')
    call s:addfilecmds("cucumber")
  endif
...
function! s:cucumberList(A,L,P)
  return s:completion_filter(rails#app().relglob("features/","**/*",".feature"),a:A)
endfunction

function! s:jasmineList(A,L,P)
  return s:completion_filter(rails#app().relglob("spec/javascripts","**/*","_spec.js"),a:A)
endfunction
...
function! s:stylesheetEdit(cmd,...)
  ...
  if rails#app().has('sass') && rails#app().has_file('app/stylesheets/'.name.'.sass')
    return s:EditSimpleRb(a:cmd,"stylesheet",name,"app/stylesheets/",".sass",1)
  elseif rails#app().has('sass') && rails#app().has_file('app/stylesheets/'.name.'.scss')
...
function! s:unittestEdit(cmd,...)
  ...
  let mapping = {'test': ['test/unit/','_test.rb'], 'spec': ['spec/models/','_spec.rb'], 'jasmine': ['spec/javascripts/', '_spec.js']}
...
function! s:cucumberEdit(cmd,...)
  if a:0
    return s:EditSimpleRb(a:cmd,"features",a:1,"features/",".feature")
  else
    call s:EditSimpleRb(a:cmd,"features","spec_helper", "features/",".feature")
  endif
endfunction

function! s:jasmineEdit(cmd,...)
  if a:0
    return s:EditSimpleRb(a:cmd,"spec",a:1,"spec/javascripts","_spec.js")
  else
    call s:EditSimpleRb(a:cmd,"spec","spec_helper","spec/javascripts",".js")
  endif
endfunction
